 ...
 * @FilePath: /ocean_roguelike/assets/script/Hero.ts
 * @Description: 注释信息
 */
...
export class Hero extends CharacterBase {
    ...
    update(deltaTime: number) {
        ...
        var radian: number;
        if (Hero.enemiesInArea.length > 0) {
            // var tempEnemy =this.enemiesInArea[Math.floor(Math.random()*this.enemiesInArea.length)];
            var tempEnemy = Hero.enemiesInArea[0];
            Hero.fireDirection = v2(tempEnemy.worldPosition.x - this.node.worldPosition.x, tempEnemy.worldPosition.y - this.node.worldPosition.y);
